...
Initial values of the global B<rgRpiRev::Global> registers are set for
...
=item B<rgRpiRev::Global>	- Global data is this class

This is the intended primary use.

The B<Global> member is a static member of this class, whose type is this
class.  This definition seems a bit recursive, but not really.  Each object
has a member B<Global> that is a reference to the same global class data.
...
disturbing the global data object B<Global>.
...
function (see below).
Note these static functions access the global data object B<Global>.
...
=head1		EXAMPLES
 ...
 // Query the global configuration
    code  = rgRpiRev::Global.RevCode.get();
    final = rgRpiRev::Global.RevCode.is_final();

    soc   = rgRpiRev::Global.SocEnum.get();
    final = rgRpiRev::Global.SocEnum.is_final();
    fail  = rgRpiRev::Global.SocEnum.is_fail();

    addr  = rgRpiRev::Global.BaseAddr.get();
    final = rgRpiRev::Global.BaseAddr.is_final();
 ...
 // Construct a local object, unrelated to global rgRpiRev::Global, which
