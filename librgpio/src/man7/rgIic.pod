...
All three have the same interface structure described here.
...
There is also an iic2 interface, but it is dedicated to the HDMI interface and
should not be accessed by user programs.  No GPIO Alternate Function pins are
identified for it.
...
essentially by disabling the pull-up FET (Field Effect Transistor).

Be aware the internal GPIO pin pull up/down resistors can be configured to
pull up, pull down, or be turned off.

Note that RPi-3 and earlier boards have 1.8 kohm pullup resistors on the
iic1 pins Gpio[2] and Gpio[3].  RPi-4 does not.  See RPi schematics.
...
version [perhaps when there were separate Rx and Tx FIFOs].
